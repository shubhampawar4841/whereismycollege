...
export default function MhetCetYearTrendsPage() {
  ...
  // Fetch rounds when course is selected
  useEffect(() => {
    fetchRounds()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [college, category, seatType, course])
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.filters) {
          ...
          // Categories and seat types will be fetched when college is selected
        }
      }
    } catch (error) {
  ...
  // Fetch rounds when college, category, seat type, and course are selected
  ...
  const handleAnalyze = async () => {
    if (!college || !category || !seatType || !course || !round) {
    ...
    try {
      ...
      const response = await fetch(`/api/mhet-cet/trends?${params}`)
  ...
  const chartData = trends.filter(t => t.found).map(t => ({
    year: t.year,
    rank: t.rank || 0,
          ...
          <div className="flex items-center gap-3 sm:gap-4 mb-4">
            ...
            <div className="min-w-0">
              ...
              <p className="text-gray-300 text-sm sm:text-base">
                Analyze how cutoff varies across years for a specific round
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className="bg-gradient-to-br from-purple-500/10 to-purple-600/10 border-purple-500/30">
          <CardHeader>
            ...
            <CardDescription className="text-xs sm:text-sm">
              Select college first, then course and round will be filtered accordingly
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 sm:space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
              ...
              <div>
                ...
                {!college || !category || !seatType || !course ? (
                  <div className="w-full px-3 sm:px-4 py-2 bg-black/30 border border-white/5 rounded-lg text-gray-500 text-xs sm:text-sm">
                    {!college ? 'Select college first' : !category ? 'Select category first' : !seatType ? 'Select seat type first' : 'Select course first'}
                  </div>
                ) : loadingOptions ? (
                  <div className="flex items-center justify-center h-10">
                    <Loader2 className="w-4 h-4 animate-spin text-purple-500" />
                  ...
                  >
                    ...
                    {rounds.length === 0 ? (
                      <option disabled>No rounds found</option>
                    ) : (
                      rounds.map((r) => (
            ...
            <button
              ...
              disabled={loading || !college || !category || !seatType || !course || !round}
            ...
            {hasData && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mt-6">
                ...
                <Card className="bg-black/20 border-white/5">
                  <CardHeader>
                    <CardTitle className="text-base sm:text-lg">Rank Trend</CardTitle>
                    <CardDescription className="text-xs sm:text-sm">
                ...
                <Card className="bg-black/20 border-white/5">
                  <CardHeader>
                    <CardTitle className="text-base sm:text-lg">Percentile Trend</CardTitle>
                    <CardDescription className="text-xs sm:text-sm">
                      {college} - {course} ({category}, {seatType})
                    </CardDescription>
                  </CardHeader>
